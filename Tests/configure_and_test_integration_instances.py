...
class Build:
    ...
    test_pack_target = '{}/project'.format(os.getenv('HOME'))
    zip_test_pack_target = f'{content_path}/artifacts'
    ...
    if LooseVersion(build.server_numeric_version) >= LooseVersion('5.5.0'):
        for server in build.servers:
            ...
            try:
                ...
                SimpleSSH(host=server.replace('https://', '').replace('http://', ''), key_file_path=Build.key_file_path, user='ec2-user').exec_command('sudo systemctl restart demisto')
...
def create_nightly_test_pack():
    init_test_pack(Build.test_pack_target)
    packs_root, dirs, _ = next(os.walk(f'{Build.content_path}/Packs'))
    for pack_dir in dirs:
        if pack_dir in SKIPPED_PACKS:
            continue
        if os.path.isdir(f'{packs_root}/{pack_dir}/TestPlaybooks'):
            playbook_root, _, test_playbooks = next(os.walk(f'{packs_root}/{pack_dir}/TestPlaybooks'))
            for playbook in test_playbooks:
                copyfile(f'{playbook_root}/{playbook}',
                         f'{Build.test_pack_target}/test_pack/TestPlaybooks/{playbook}')
    make_archive(f'{Build.test_pack_target}/test_pack', 'zip', Build.zip_test_pack_target)
    rmtree(f'{Build.test_pack_target}/test_pack')


def create_pack_metadata(export_to):
    export_to = f'{export_to}/metadata.json'
    ...
    with open(export_to, 'w') as metadata_file:
        metadata_file.write(json.dumps(metadata))


def init_test_pack(target):
    pack_path = f'{target}/test_pack'
    os.mkdir(pack_path)
    os.mkdir(f'{pack_path}/TestPlaybooks')
